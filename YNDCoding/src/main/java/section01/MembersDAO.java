...
public class MembersDAO {
	...
	public HashMap<String, Object> getOneMemberIdName(String m_id) {
		...
		try(
			...
			PreparedStatement pstmt = conn.prepareStatement("select m_id, m_name, m_profile, m_class from members where m_id=?");
		) {
		...
		public List<MemberBean> SelectMembersList() {
		      ...
		      try {
		         ...
		         while (cur.next()) {
		            ...
		            Blob mprofile = cur.getBlob(7);
		            MemberBean member = new MemberBean(mid, mpw, mname, mpwq, mpwa, mphone, mprofile, mclass);
		            list.add(member);
		         }
		         cur.close();
		         stmt.close();
